...
func (d *Teldrive) request(method string, pathname string, callback base.ReqCallback, resp interface{}) error {
	...
	if _req.Status() == "401 Unauthorized" {
		e := d.refreshToken()
		if e != nil {
			return e
		}
		return d.request(method, pathname, callback, resp)
	}
	if _req.IsError() {
		return &e
	}
...
func (d *Teldrive) refreshToken() error {
	var sessionResp SessionResp
	resp, err := base.RestyClient.R().SetHeader("Cookie", d.Cookie).SetResult(&sessionResp).Execute("GET", d.Address+"/api/auth/session")
	if err != nil {
		return err
	}
	if resp.IsError() || resp.StatusCode() != 200 {
		return fmt.Errorf("[Teldrive] refresh token failed: %s", resp.String())
	}
	
	d.Cookie = resp.Header().Get("Set-Cookie")
